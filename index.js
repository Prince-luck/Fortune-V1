...
const Wolf = WolfConnect({
    ...
    auth: state,
    browser: ['Føřťůŋɛ-MD-V1', 'safari', '3.3'],
    ...
    getMessage: async (key) => {
                ...
                return { conversation: "Føřťůŋɛ-MD-V1 🙂⃤ 🙂⃤ταιяα мακιиο" };
    }
})
...
Wolf.setStatus = (status) => {
   ...
   Wolf.ev.on('connection.update', async (update) => {
        ...
        if (connection === "open" || update.receivedPendingNotifications == "true")  {
          ...
          console.log("Welcome to Føřťůŋɛ-MD-V1 ✨");
          const userName = Wolf.user.name ? Wolf.user.name : global.BotName;
          console.log('♱ Føřťůŋɛ-MD-V1 User Info');
          console.log(`♱ Name     : ${userName}`);
          console.log(`♱ Number   : ${Wolf.user.id.split(':')[0]}`);
          console.log(`♱ Status   : Connected`);
          const packageVersion = require("./package.json").version;
          const long = String.fromCharCode(8206);
          const readmore = long.repeat(4001);  
          let uinfo = `
          ♱ Føřťůŋɛ-MD-V1 User Info
          ♱ Name     : ${userName}
          ♱ Number   : ${Wolf.user.id.split(':')[0]}
          ♱ Status   : Connected
